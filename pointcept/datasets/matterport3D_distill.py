...
class MatterPort3DDistillDataset(DefaultDataset): # 혹은 ScanNetDatasetBoundary
    def __init__(
        self,
        boundary_root=None,
        features_root=None, #
    ...
    def get_data(self, idx):
        ...
        if self.features_root is not None and len(self.features_flag) > 0:
            expected_dim = len(self.features_flag) * np.array([3 if f == "scale" else 1 if f == "opacity" else 4 for f in self.features_flag]).sum()
            ...
            if os.path.exists(features_file_path):
                ...
                data_dict["features"] = np.zeros((data_dict["coord"].shape[0], expected_dim), dtype=np.float32)
